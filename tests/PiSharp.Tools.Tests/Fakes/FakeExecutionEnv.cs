...
namespace PiSharp.Tools.Tests.Fakes;

internal sealed class FakeExecutionEnv : IExecutionEnv
{
    private readonly Dictionary<string, string> _files = new(StringComparer.OrdinalIgnoreCase);
    private readonly HashSet<string> _directories = new(StringComparer.OrdinalIgnoreCase);
    private readonly Queue<Func<string, ExecutionOptions?, ShellResult>> _shellResults = new();
    ...
    public void EnqueueShellResult(string stdout, string stderr = "", int exitCode = 0)
        => _shellResults.Enqueue((_, options) =>
        {
            if (stdout.Length > 0)
            {
    ...
    {
        if (cancellationToken.IsCancellationRequested) return ExecErr(ExecutionErrorCode.Aborted, "Operation aborted");
        if (_shellResults.Count == 0) return ExecErr(ExecutionErrorCode.SpawnError, $"No fake shell result queued for command: {command}");
        try { return Task.FromResult(Result<ShellResult, ExecutionError>.Ok(_shellResults.Dequeue()(command, options))); }
        catch (Exception exception) { return ExecErr(ExecutionErrorCode.Unknown, exception.Message, exception); }
    }
    ...
    private static Task<Result<ShellResult, ExecutionError>> ExecErr(ExecutionErrorCode code, string message, Exception? cause = null) => Task.FromResult(Result<ShellResult, ExecutionError>.Err(new ExecutionError(code, message, cause)));
}
