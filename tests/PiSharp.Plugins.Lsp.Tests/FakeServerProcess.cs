...
/// <see cref="StandardOutput"/> are Pipe-backed streams, and the fake server side reads
/// client writes from <see cref="ServerInput"/> and writes responses/events to
...
public sealed class FakeServerProcess(ProcessStartInfo startInfo) : IServerProcess
{
    ...
    /// <summary>Fakes the adapter emitting a stderr line.</summary>
...
/// ProcessStartInfo the production code built. Each <see cref="NextProcessAsync"/> await
/// returns the next spawned process (a FIFO of pending spawns), so multi-session tests
/// bind one scripted peer per process.
/// </summary>
public sealed class FakeServerProcessFactory : IServerProcessFactory
{
    ...
    /// <summary>
    /// Completes with the next spawned process (queueing up more awaits for further
    /// spawns). Always completes; the order matches spawn order.
    /// </summary>
    ...
    public IServerProcess Start(ProcessStartInfo startInfo)
    {
        ...
        lock (_gate)
        {
            if (_waiters.Count > 0)
            {
                _waiters.Dequeue().TrySetResult(process);
            }
            else
            {
