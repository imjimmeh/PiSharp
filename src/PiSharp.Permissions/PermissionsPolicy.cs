    ...
    public PermissionDecision Evaluate(string tool, string serializedArgs, string dangerousCategory, bool headless)
    {
        ...
        var (defaultAction, defaultReason) = dangerousCategory switch
        {
            ...
            DangerousOpDetector.GitPush => (PermissionAction.Ask,
                "Bash command contains a destructive git operation (push or reset --hard); approval required."),
            DangerousOpDetector.RmRf => (PermissionAction.Ask,
                "Bash command recursively force-deletes files (rm -rf); approval required."),
            DangerousOpDetector.McpSpawn => (PermissionAction.Ask,
                $"Tool '{tool}' is an MCP tool that can execute a command; approval required."),
            DangerousOpDetector.Unknown => (PermissionAction.Ask,
                $"Tool '{tool}' is not recognized as a safe built-in tool; approval required."),
